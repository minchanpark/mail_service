...
type ResourceKey = "auth" | "accounts" | "threads" | "labels" | "briefing" | "providers";
...
type BackendAdapter = {
  ...
  threads: {
    list: (filter: ThreadFilter) => Promise<{ items: Thread[] }>;
    get: (id: string) => Promise<Thread | null>;
    update: (id: string, patch: Partial<Thread>) => Promise<Thread>;
    subscribe: (filter: ThreadFilter, callback: (items: Thread[]) => void) => Unsubscribe;
  };
  ai: {
...
export function createHttpBackend(): BackendAdapter {
  ...
  return {
    ...
    threads: {
      list: (filter) => request<{ items: Thread[] }>(buildThreadListUrl(filter)),
      ...
      subscribe(filter, callback) {
        ...
        const load = async () => {
          const result = await request<{ items: Thread[] }>(buildThreadListUrl(filter));
...
export type { BackendAdapter, ResourceKey };
